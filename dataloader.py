...
class dialogDataset(Dataset):
    def __init__(self, file_name, tokenizer_address='bert-base-uncased', max_length=80, data_size=1):
        self.tokenizer = BertTokenizer.from_pretrained(tokenizer_address)

        # print('target_token_id=', self.tokenizer.mask_token_id)
        ...
        #     if 'D2xXb7UF' in self.ids[i]:
        #         print(self.ids[i] + ' : ' + str(self.dialog[i]) + str(self.role[i]) + str(self.label[i]) + str(self.edge[i]))
