...
# from model_remove_textcnn import BugListener
...
def get_dialog_loaders(config, train_address, test_address, pretrained_model, batch_size=32, num_workers=0, pin_memory=False, data_size=1):
    train_set = dialogDataset(train_address + '_train.json', pretrained_model, data_size=data_size)
...
class Config(object):
    def __init__(self, project):
        ...
        # self.pretrained_model = './../pretrained_model/roberta_base'
        # self.pretrained_model = './../pretrained_model/deberta_base'
        self.D_bert = 768
    ...
    for data_size in [0.2, 0.4, 0.6, 0.8, 1]:
        for project in ['angular', 'appium', 'docker', 'dl4j', 'gitter','typescript']:
            ...
            train_loader, test_loader = get_dialog_loaders(config, config.train_address, config.test_address,
                                                           ...
                                                           batch_size=config.batch_size, data_size=data_size)
            ...
            # graph_loss = SCELoss(num_classes=config.graph_class_num)
            # graph_loss = LabelSmoothingCrossEntropy()
